...
const getService = (req: Request) => {
  ...
  return new CashMovementService(prisma);
};
...
export const getCashMovement = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const updateCashMovement = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!id || typeof id !== 'string') throw new AppError('ID requerido', 400);
  const service = getService(req);
