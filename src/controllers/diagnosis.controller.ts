...
const getService = (req: Request) => {
  ...
  return new DiagnosisService(prisma);
};
...
export const listDiagnoses = asyncHandler(async (req: Request, res: Response) => {
  const service = getService(req);
  const result = await service.list();
  res.json({ ok: true, data: result });
});

export const getDiagnosis = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const updateDiagnosis = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!id || typeof id !== 'string') throw new AppError('ID requerido', 400);
  const service = getService(req);
...
export const deleteDiagnosis = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!id || typeof id !== 'string') throw new AppError('ID requerido', 400);
  const service = getService(req);
